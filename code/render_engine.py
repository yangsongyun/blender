...
def create_roughness_map(filepath, working_dir, params_current_shader, current_object, npy=False):
    ...
    bpy.ops.render.render()
    bpy.data.images['Render Result'].save_render(
        filepath=os.path.join(filepath, 'roughness.exr'))

    bpy.context.scene.use_nodes = False
    mat.node_tree.links.new(inp, outp)


def create_specular_map(filepath, working_dir, params_current_shader, current_object, npy=False):
    bpy.context.scene.render.image_settings.color_mode = 'BW'
    bpy.context.scene.render.image_settings.file_format = 'OPEN_EXR'
    bpy.context.scene.render.image_settings.color_depth = '32'
    bpy.context.scene.render.image_settings.exr_codec = 'NONE'
    ...
    outp = pbs_node.outputs['BSDF']
    for key, value in params_current_shader.items():
        if key.lower() in ["specular_texture"]:
            texture_node = mat.node_tree.nodes.new('ShaderNodeTexImage')
            if not os.path.exists(value):
                value = os.path.join(working_dir, value)
                print("添加贴图路径" + value)
    ...
    bpy.ops.render.render()
    bpy.data.images['Render Result'].save_render(
        filepath=os.path.join(filepath, 'specular.exr'))
...
def create_metallic_map(filepath, working_dir, params_current_shader, current_object, npy=False):
    ...
    bpy.ops.render.render()
    bpy.data.images['Render Result'].save_render(
        filepath=os.path.join(filepath, 'metallic.exr'))
...
def create_diffuse_map(filepath, working_dir, params_current_shader, current_object, npy=False):
    ...
    bpy.ops.render.render()
    bpy.data.images['Render Result'].save_render(
        filepath=os.path.join(filepath, 'diffuse.exr'))
...
def render_object(object_shape, params_rendering, params_camera, params_object_global,
    ...
    for cidx , pose in enumerate(camera_poses_set):
        ...
        for tid, texture_path in enumerate(area_light_textures):
            ...
            for j in light_names:
                ...
                area_texture_node.image = bpy.data.images.load(texture_path)
            ...
            bpy.context.scene.render.image_settings.exr_codec = 'NONE'
            bpy.ops.render.render()
            filepath = os.path.join(out_dir_v, '{:03d}.exr'.format(tid + 1))
        ...
        create_roughness_map(filepath=out_dir_v, working_dir=working_dir, params_current_shader=params_shader[i],
                             current_object=obj_names[i], npy=enable_npy)
        create_metallic_map(filepath=out_dir_v, working_dir=working_dir, params_current_shader=params_shader[i],
                            current_object=obj_names[i], npy=enable_npy)
        create_specular_map(filepath=out_dir_v, working_dir=working_dir, params_current_shader=params_shader[i],
                            current_object=obj_names[i], npy=enable_npy)
        create_diffuse_map(filepath=out_dir_v, working_dir=working_dir, params_current_shader=params_shader[i],
                           current_object=obj_names[i], npy=enable_npy)
